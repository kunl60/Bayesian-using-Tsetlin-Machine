...
from data.generate_real_categorical import load_neticusdroid_dataset

# 1. Setup Data - row-subsampled to 3000 (was the full 29332 rows, unbounded)
full_df = load_neticusdroid_dataset(sample_size=3000, seed=42)
full_df.columns = [f"col{i+1}" for i in range(len(full_df.columns))]
...
for run in range(1, 6):
    ...
    train_df, test_df = train_test_split(full_df, test_size=0.2, random_state=run)
    ...
    print(f"SVC Mean Accuracy: {100 * np.mean(svc_accuracies):.2f}% ± {100 * np.std(svc_accuracies):.2f}%")
    print(f"Naive Bayes Mean Accuracy: {100 * np.mean(nb_accuracies):.2f}% ± {100 * np.std(nb_accuracies):.2f}%")
    print(f"KNN Mean Accuracy: {100 * np.mean(knn_accuracies):.2f}% ± {100 * np.std(knn_accuracies):.2f}%")

    # Markov Blanket stats (same each run, but printed as requested)
    mb_sizes = [len(members) for members in MB_results.values()]
    print("\n--- Markov Blanket Stats ---")
    print(f"Average MB size per node: {np.mean(mb_sizes):.2f}")
    print(f"Std MB size per node: {np.std(mb_sizes):.2f}")
