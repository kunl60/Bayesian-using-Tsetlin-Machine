...
from algorithms.mi_bn import MutualInfoMBElbow
from sklearn.svm import SVC
from sklearn.naive_bayes import CategoricalNB
from data.generate_real_categorical import load_mushroom_dataset

# 1. Setup Data - row-subsampled to 3000 (was the full ~8124 rows, unbounded)
encoded_df = load_mushroom_dataset(sample_size=3000, seed=42)
...
n_categories = encoded_df.nunique().to_dict()

# 2. Discover Markov Blankets using the elbow-cut mutual-information variant
# (candidate count per target picked via find_elbow_index instead of a
# fixed top_n) - see algorithms/mi_bn.py. Same shared pipeline as
# TsetlinMB_elbow (algorithms/mb_bn_base.py), only the scoring step
# differs (I(V_j; Y) instead of clause frequency x weight).
mi_mb_elbow = MutualInfoMBElbow(top_n=5)
discovery_results = mi_mb_elbow.run(encoded_df, encoded_df.astype(str))

# Extract MB_results from the graph K
K = discovery_results["model"]
MB_results = {node: list(K.neighbors(node)) for node in K.nodes()}

# 4. Evaluate over multiple runs, each with a different but fixed split seed
...
for run, seed in enumerate(run_seeds, start=1):
    train_df, test_df = train_test_split(encoded_df, test_size=0.2, random_state=seed)
