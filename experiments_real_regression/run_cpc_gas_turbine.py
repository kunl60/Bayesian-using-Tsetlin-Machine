"""
CPC run on Gas Turbine CO and NOx Emission (UCI), a real-world industrial
sensor-signal regression dataset with an unknown ground-truth DAG. Discovers
the skeleton/Markov Blankets via CPC (a causal-learn based PC/FCI variant with
kPC-style PAG orientation rules), then scores each node's discovered MB (and,
for comparison, all raw features) as predictors for that node via Linear
Regression / KNN / SVR (see experiments_real_regression/_common.py).
"""
...
def main():
    ...
    print("===== CPC Results (Fisher-Z, Gas Turbine Emission) =====")
