...
class MutualInfoMBElbowContinuousRegressor:
    ...
    def compute_target_PC(self, target, candidates_with_scores, raw_samples, separating_sets):
        ...
        # anything the scorer selected that is in fact unconditionally
        # independent of the target before spending combinatorial tests.
        PC = []
        for X in score:
    ...
    def _orient_edge(dag, u, v):
        ...
        if has_uv and has_vu:
            if MutualInfoMBElbowContinuousRegressor._would_create_cycle(dag, u, v):
    ...
    def _score_candidates(self, raw_samples):
        columns = raw_samples.columns.tolist()
        raw_scores = {}

        for target in columns:
            y = raw_samples[target].astype(np.float64)
            if y.nunique() < 2:
                # Constant target column - nothing to score against it.
                continue

            feature_cols = [c for c in columns if c != target]
            X = raw_samples[feature_cols].astype(np.float64)
    ...
    def run(self, samples, raw_samples):
        ...
        raw_scores = self._score_candidates(raw_samples)
        ...
        # See wtm_bn_regressor.py for the full rationale behind
        # this hybrid sepset-reuse strategy - unchanged here.
