...
def main():
    ...
    for run, seed in enumerate(run_seeds, start=1):
        ...
        # top_n set to the full variable count (26 = 27 nodes - target) so
        # no candidate input is cut - conditioning sets are then searched
        # up to size 3 (maxK) over a two-sided, shrinking PC pool per
        # target - see TsetlinMB.compute_target_PC.
        tm_mb = TsetlinMB(num_epochs=5, number_clauses=10, top_n=5, maxK=3, T=10, s=5)
        results = tm_mb.run(data, samples)
